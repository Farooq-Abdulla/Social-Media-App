...
export default async function Page({ params }: PageProps) {
    ...
    return (
        <main className="flex w-full min-w-0 gap-5">
            ...
            <div className="sticky top-[5.25rem] hidden md:block lg:block w-80 h-fit flex-none">
        ...
        <div className="space-y-5 rounded-2xl bg-card p-5 shadow-lg">
            ...
            <UserTooltip user={user}>
                <Link href={`/users/${user.displayName}`} className="flex items-center gap-3">
                    <UserAvatar avatarUrl={user.image} className="float-none" />
                    <div>
                        <p className="line-clamp-1 break-all font-semibold hover:underline ">{user.displayName}</p>
                        <p className="line-clamp-1 break-all text-muted-foreground">@{user.name}</p>
                    </div>
                </Link>
            </UserTooltip>
            <Linkify>
